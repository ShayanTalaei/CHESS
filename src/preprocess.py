...
def process_database(db_id: str, args: argparse.Namespace):
    """
    Processes a single database to create LSH and context vectors for a given database ID.
...
if __name__ == '__main__':
    ...
    print(type(args.db_root_directory))
    # if args.db_id == 'all':
    #     for db_id in os.listdir(args.db_root_directory):
    #         process_database(db_id, args)
    # else:
    #     process_database(args.db_id, args)
    process_database(args.db_id, args)
    logging.info("Preprocessing is complete.")
