...
class DatabaseSchemaGenerator:
    ...
    def _set_foreign_keys(db_path: str, database_schema: DatabaseSchema) -> None:
        ...
        for table_name, columns in schema_with_references.items():
            ...
            for fk in foreign_keys_info:
                ...
                schema_with_references[source_table][source_column]["foreign_keys"].append(
        ...
        for table_name in db_schema.tables.keys():
            ...
            for col in columns:
                ...
                unique_values = execute_sql(
                    db_path,
                    f"SELECT COUNT(*) FROM (SELECT DISTINCT `{col[1]}` FROM `{table_name}` LIMIT"
                    " 21) AS subquery;",
                    "all",
                    480,
                )
                ...
                if is_categorical:
                    unique_values = execute_sql(
                        db_path,
                        f"SELECT DISTINCT `{col[1]}` FROM `{table_name}` WHERE `{col[1]}` IS NOT"
                        " NULL",
                    )
                ...
                try:
                    ...
                    print(
                        f"An error occurred while fetching statistics for {col[1]} in"
                        f" {table_name}: {e}"
                    )
        ...
        for table_name, table_schema in self.schema_structure.tables.items():
            for column_name, column_info in table_schema.columns.items():
                ...
                if (self.add_examples and not column_info.examples) or (
                    (column_info.type.lower()) == "date" or ("date" in column_name.lower())
                ):
                    example = execute_sql(
    ...
    def _get_example_column_name_description(
        ...
        if column_info:
            if column_info.examples:
                example_part = (
                    f" Example Values: {', '.join([f'`{str(x)}`' for x in column_info.examples])}"
                )
    ...
    def get_column_profiles(
        ...
        for table_name, table_schema in self.schema_structure.tables.items():
            ...
            for column_name, column_info in table_schema.columns.items():
                if with_keys or not (
                    column_info.primary_key or column_info.foreign_keys or column_info.referenced_by
                ):
                    column_profile = (
                        f"Table name: `{table_name}`\nOriginal column name: `{column_name}`\n"
                    )
                    if (
                        column_info.column_name.lower().strip() != column_name.lower().strip()
