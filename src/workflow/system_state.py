...
class SystemState(BaseModel):
    ...
    def add_columns_to_tentative_schema(self, selected_columns: Dict[str, List[str]]) -> None:
        ...
        for table_name, columns in selected_columns.items():
            target_table_name = next(
                (t for t in self.tentative_schema.keys() if t.lower() == table_name.lower()), None
            )
            if target_table_name:
                for column in columns:
                    if column.lower() not in [
                        c.lower() for c in self.tentative_schema[target_table_name]
                    ]:
    ...
    def check_schema_status(self) -> Dict[str, any]:
        ...
        for table_name, cols in correct_columns.items():
            for col in cols:
                selected_table = [
                    table
                    for table in self.tentative_schema.keys()
                ...
                if not selected_table:
                    ...
                    if col.lower() not in [
                        selected_col.lower()
                        for selected_col in self.tentative_schema[selected_table]
                    ]:
    ...
    def construct_history(self) -> Dict:
        ...
        for index in range(len(values_list) - 1):
            ...
            history += (
                "Feedbacks:"
                f" {self.remove_new_lines(self._get_feedback_string(values_list[index][0].feedbacks))}\n"
            )
