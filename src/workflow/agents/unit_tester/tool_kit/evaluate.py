...
class Evaluate(Tool):
    ...
    def _run(self, state: SystemState):
        ...
        for index, candidate_query in enumerate(target_SQL_meta_infos):
            formatted_candidates += (
                f"Candidate Response #{index+1}: Query: {candidate_query.SQL}\n, Execution Result:"
                f" {self._format_sql_query_result(candidate_query)}\n"
            )
    ...
    def test_case_filtering_based_on_inter_cluster_variance(
        self, candidates_clusters: Dict, scores: List[int], target_SQL_meta_infos: List[SQLMetaInfo]
    ) -> bool:
        """
        ...
        for key, candidates in candidates_clusters.items():
            cluster_scores = [
                scores[target_SQL_meta_infos.index(candidate)] for candidate in candidates
            ]
    ...
    def pick_the_best_candidate(
        ...
        best_candidates = [
            candidates[index] for index, score in enumerate(scores) if score == max_score
        ]
